...
test('derive another proxy', async () => {
  const computeDouble = vi.fn((x: number) => x * 2);
  const state = proxy({
    text: '',
    count: 0,
...
test('derive with two dependencies', async () => {
...
test('derive with array.pop', async () => {
  const state = proxy({
    arr: [{ n: 1 }, { n: 2 }, { n: 3 }],
  });
  derive(
...
describe('two derived properties', () => {
  ...
  test('two derived properties both returning primitive values (#349)', async () => {
  ...
  test('two derived properties both returning non primitive values, defined at the same time (#349)', async () => {
    const state: State = proxy({ a: 1 });
    derive(
      {
        derived1: (get) => {
  ...
  test('two derived properties both returning non primitive values (#349)', async () => {
    const state: State = proxy({ a: 1 });
    derive(
      {
        derived1: (get) => {
